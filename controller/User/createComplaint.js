...
console.log({ evidencePictures})
    ...
    var options = {
      maxTimeMS: 2000000, // Set the timeout in milliseconds (adjust as needed)
    };
...
// Promise.all(processEvidencePictures).then(async function () {
...
//       let rawData = await fs.readFileSync(liveImage.path);
      
//       await fs.writeFile(newPath, rawData, async function (err) {
//           if (err) {
//               console.error("Error writing file:", err);
//               return res.status(500).send("Error writing file");
//           }
...
//          console.log(user)
//          await user.save(options).then(async (profile) => {
...
//              <strong>maxDecibelVideo:</strong>  ${maxDecibel == 'Allow visualizer' ? "Access was not given to visualizer" : videoCloud || liveVideoCloud ? Number( maxDecibel && maxDecibel.split(' ')[0]) + 9 : `No Video`}<br>
//              </div>
//             <br>
//             <br>
...
//              <strong>maxDecibelAudio:</strong>  ${maxDecibel == 'Allow visualizer' ? "Access was not given to visualizer" : maxDecibel}<br>
